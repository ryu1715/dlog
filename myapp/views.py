...
def Like_add(request, post_id):
    ...
    is_liked = Like.objects.filter(user=request.user, post=post.id).count()
    if is_liked > 0:
        messages.info(request, 'すでにお気に入り済みです')
...
def Search(request):
    if request.method == 'POST':
        ...
        if searchform.is_valid():
            ...
            search_list = Post.objects.filter(Q(title__icontains=freeword)
                                              | Q(content__icontains=freeword))
